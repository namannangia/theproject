    ...
    try {
      ...
      if (response && response.status === 200) {
        ...
        try {
          ...
          if (response2 && response2.data && response2.data.userdata) {
            console.log("Login successful. Response:", response2.data.userdata);
            ...
            console.log("Login failed. Invalid response2.", response2);
            return false;
          }
        } catch (error) {
          ...
          if (resp.data.lectures.length === 0) return 404;
            ...
            try {
              ...
              for (x of resp.data.lectures) {
                var res = await Api.get("/teacher/getLectureById/" + x);
                if (res) payload.push(res);
              }
              return payload;
            } catch (error) {
              console.log("Authservice.getLectures (insider):", error);
            }
          }
        }
      } catch (error) {
